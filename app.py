...
# ─────────────────────────────────────────────────────────────────────────────
# КОНСТАНТЫ (в т.ч. для исправления Pylance)
# ─────────────────────────────────────────────────────────────────────────────
JOURNAL_PATH = "data/portfolio_journal.csv"  # ✅ Добавлено: явное определение пути
...
with tab3:
    ...
    with st.expander("🤖 Как работает жадный алгоритм подбора?"):
        ...
        2. **Ограничение доли**: `Лимит = (Текущий портфель + Бюджет) × Макс.% - Уже_вложено`.\n
        3. **Приоритет доходности**: Кандидаты сортируются по YTM.\n
        4. **Целевое распределение**: Лестница → по годам, Штанга → 50/50, Колесо → циклично.\n
        5. **Жадное заполнение остатка**: Если >10% бюджета свободно, алгоритм покупает следующие лучшие лоты.
        """)
...
with tab4:
    ...
    st.markdown("История подтверждённых покупок. При расчёте лимитов система учитывает эти данные.")
    ...
    if st.button("🗑 Очистить журнал (для нового теста)"):
        Path(JOURNAL_PATH).unlink(missing_ok=True)  # ✅ Теперь переменная определена выше
